<template>
  <div class="mod-config">
      <!--  这里走所有产品中绑定的仓库信息-->
    <el-form :inline="true" :model="dataForm" @keyup.enter.native="getProductHoseNotNull()">
      <el-form-item>
          <!-- 这里需要查询仓库表中，回传信息，前台做封装，这里需要在打开页面的时候就需要进行查询返回封装数据 -->
        <el-select v-model="dataForm.key" placeholder="请选择仓库" >
          <el-option
      ...
      <el-form-item>
        <el-button @click="getProductHoseNotNull()">查询</el-button>
      </el-form-item>
    </el-form>
    <el-table
      :data="dataList"
    ...
    >
      ...
      <el-table-column prop="capacity" header-align="center" align="center" label="仓库容量"></el-table-column>
      <el-table-column prop="occupied" header-align="center" align="center" label="已占用容量"></el-table-column>
...
export default {
  ...
  methods: {
    ...
    getDataList() {
      ...
      this.$http({
        url: this.$http.adornUrl("/wms/warehouse/list"),
        method: "get",
    ...
    deleteHandle(id) {
      ...
      this.$confirm(
        ...
        this.$http({
          url: this.$http.adornUrl("/wms/warehouse/delete"),
          method: "post",
